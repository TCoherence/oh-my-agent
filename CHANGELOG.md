...
- **Configurable dashboard auto-refresh interval; default raised from 60s to 300s (5 min)**. Original PR #32 hardcoded a 60-second `<meta http-equiv="refresh">` tag in the template. Field feedback: 60s is too aggressive when most data sections only change at the minute granularity, and there was no knob to tune it. This PR makes the interval configurable end-to-end. New `--refresh-seconds N` CLI flag on `oma-dashboard` (resolution order: CLI flag → `OMA_DASHBOARD_REFRESH_SECONDS` env var → default 300). Pass `0` to disable the meta-refresh tag entirely (page becomes manual-refresh-only). Negative values are clamped to 0 (defensive). `create_app(config, *, refresh_seconds=300)` is now a keyword arg with a default. The Jinja2 template wraps the meta tag in `{% if refresh_seconds and refresh_seconds > 0 %}` so disabling actually omits the tag rather than emitting a buggy `content="0"`. The header line above the section grid switches from a hardcoded `auto-refresh every 60s` to a dynamic `auto-refresh every {{ refresh_seconds }}s` (or `auto-refresh disabled` when off), so operators always see the actual configured value. The startup banner (`oma-dashboard: serving on http://...`) gains an `auto-refresh=every Ns` clause. `scripts/docker-common.sh:oma_dashboard_build_run_args` forwards `OMA_DASHBOARD_REFRESH_SECONDS` from host environment to the dashboard container — set the env on the host shell, every restart picks it up. monitoring.md §6.3 (EN + CN) gains a one-liner showing `OMA_DASHBOARD_REFRESH_SECONDS=120 bash scripts/docker-start.sh`. Covered by 6 new tests in `test_dashboard_app.py` (default 300, custom value, zero-omits-tag, negative-clamped) + 6 new tests in `test_dashboard_cli.py` (`_resolve_refresh_seconds` precedence: default → env → CLI flag → invalid env → zero kept → negative clamped).
