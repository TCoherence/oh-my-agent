...
- **Scheduler now uses a central wall-clock due scanner instead of per-job long sleepers.** Replaces the "one asyncio task per job, each doing `asyncio.sleep(delay)`" model with a single due loop that ticks every ≤30s (`due_loop_max_tick_seconds`), reads wall clock, and dispatches any job whose `next_fire_at` has passed. After host suspend (e.g. Docker-on-Mac laptop sleep), scheduled jobs now recover within one scanner tick instead of being silently skipped by the watchdog (the prior `restart_job` per-job missed_fire path cancelled the stuck monotonic-clock sleeper and recomputed `next_fire_at` for *tomorrow*, dropping today's run). A wakeup `asyncio.Event` (`_due_loop_wakeup`) closes the latency gap for short-interval jobs — `_fire_job_once.finally`, `_start_job`, `_stop_job`, reload reconcile, and `restart_due_loop` all set it. No-backfill semantics unchanged: one fire per overdue window. Behavior changes for `/automation_run`: (a) returns a typed `FireJobResult` literal (`"ok"` / `"already_firing"` / `"not_found"` / `"scheduler_down"`) and renders an actionable message instead of a vague boolean; (b) returns immediately after dispatch (was: waited for the fire's `on_fire` enqueue step); (c) refuses if the job is already firing — manual fires while a scheduled fire is in-flight are now skipped (was: ran concurrently). `fire_job_now` for a sleeping job preserves the originally-scheduled future `next_fire_at` (`preserve_next_fire=True`) so a manual run mid-window doesn't shift the regular cadence; if the pinned time is already overdue, falls through to `completion+interval` as normal. New public APIs: `Scheduler.restart_due_loop(*, reason)`, `Scheduler.get_due_loop_runtime_state()`, `Scheduler.compute_display_next_run_at(name)`. Removed: `Scheduler.restart_job(name, *, reason)` (per-job sleepers no longer exist). The supervisor (`GatewayManager._run_scheduler_supervisor`) now dispatches `due_loop`-scope findings to `restart_due_loop`, demotes `job`-scope `missed_fire` findings to log-only (the due loop is the recovery path), and keeps the rate-limited reload-loop restart path. `_job_tasks` renamed to `_fire_tasks` to reflect the new role (in-flight fires only). All `datetime.now(self._timezone)` reads inside the Scheduler instance now route through `self._now()` for consistent fake-clock test behavior. Follow-up (out of scope): persisted `next_run_at` becoming startup-authoritative for catch-up after process restart.
