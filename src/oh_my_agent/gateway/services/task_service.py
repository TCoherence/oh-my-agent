    ...
    async def _decide_replace(
        ...
        *,
        ...
        if result == "ok":
            ...
            suffix = "already firing — refire skipped"
        elif result == "scheduler_down":
            suffix = "refire skipped (scheduler not running)"
        else:
            suffix = "refire returned not_found (no active job?)"
        return TaskActionResult(
            success=result == "ok",
