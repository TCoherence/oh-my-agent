...
- [ ] **`_execute_task_loop` 在 `completion_mode=reply` artifact task 上 post-completion hang**：2026-05-03 在 `4c59b02a6806`（market-briefing-daily-ai-0900 手动 `/automation_run`）上**可重现**。两次运行（原始 + 重启 requeue 后）都呈现相同特征：(a) claude-cli 干净结束（`terminal_reason=completed`，exit 0，4 个 sub-section 文件全落盘）；(b) DB 行无限期停在 `RUNNING`（第一次 5800s+，第二次仍在持续）；(c) **既没有 `task.completed` 事件也没有 `task.notification_failed` 事件** —— `runtime/service.py:3242` 的 COMPLETED 分支 `await self._notify(...)` 既不返回也不抛异常（所以 `service.py:3295` 的成功路径标 COMPLETED 没触发，`service.py:3251` 的 except 路径标 FAILED 也没触发）；(d) `automation_posts` 没有写行；(e) 第二次运行的 `task.agent_progress` 心跳事件在 60s 之后就没了（vs 第一次运行 18 min 期间一直有）—— 强烈暗示整个 `_execute_task_loop` asyncio task 在 ~60s 时静默 crash/cancel，但 claude 子进程独立跑完了。**重现签名**：requeue-after-hang 触发第二次运行；第二次很快（12 turn / 287s / $0.97，Stage 2.2 checkpoint recovery 起作用）但卡在同一处。同一天的其他 ai-0900 task（`4c1fd715086f` 17:15 UTC）和无关 automation（`bdcf9908d735` weekly 的 FAILED-via-timeout 路径、`0186254e737d` youtube weekly）都正常完成并写了 `automation_posts`，所以 bug 特定于**重新 claim 后的 task 走 COMPLETED-via-reply 分支**这条路径。过去 30 天 automation task 里搜匹配 pattern（agent log 完成 + exit 0 但 DB ≠ COMPLETED）= 0 次，今天是首次重现。调查切面：(1) `memory/store.py:1842` 的 `requeue_inflight_runtime_tasks` 是否在 `_running_tasks` / `_task_sources` / `self._workers` 留下与 requeued 行不一致的内存状态，导致 post-completion notify 路径 await 在某个死 worker 的 future？(2) agent 最后的结构化摘要 chat reply 是否撞上 Discord rate-limit + chunk-message 无限 retry？(3) `_emit_automation_terminal_push`（push 通知 fan-out）是否阻塞在永不 resolve 的 HTTP future？需要 docker stderr / asyncio debug log 在 21:27-21:35 UTC 窗口的内容才能精确定位。临时 workaround：重启 container + 手动 `update runtime_tasks set status='FAILED', error='hung post-completion' where id='<id>'` 打破 requeue 循环。
