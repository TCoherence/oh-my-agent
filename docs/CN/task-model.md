...
**归档行为（v0.9.3 准备中新增）**：`reply` 模式下每个 artifact 文件会被复制到 `<reports_dir>/artifacts/<filename>`。如果已有同名文件，新副本追加 `-<task_id[:8]>` 后缀。把 `runtime.reports_dir` 设为 `""` 可关闭归档。
...
3. `_archive_artifact_files()` 把每个文件复制到 `<reports_dir>/artifacts/`。有同名文件时加 `-<task_id[:8]>` 后缀；失败只打 warning，不影响任务。
...
6. **Docker 卷挂载路径。** 容器里归档落在 `/home/.oh-my-agent/reports/artifacts/`；宿主机上一般是 `${OMA_DOCKER_MOUNT:-~/oh-my-agent-docker-mount}/.oh-my-agent/reports/artifacts/`。
