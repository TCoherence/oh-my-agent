...
- `v0.9.6` is released. The 1.0 contract freeze is essentially in place; remaining work is hardening + the v1.0 acceptance criteria checklist in `v1.0-plan.md`. **Current next target: `v1.0` stable.**
- `v0.7.3` released (phases 1–3 complete); `v0.8.0` released (all four hardening phases complete); `v0.8.1` released (memory quality pass, skill contract hardening, podcast integration, automation YAML fixes); `v0.9.0` released (BREAKING memory rewrite to Judge); `v0.9.1`–`v0.9.3` released (Slack stub removal, restart/recovery, experimental-surface cleanup); `v0.9.4` released (streaming, push notifications, watchdog, central scheduler due-loop, dump channels, dashboard); `v0.9.5` released (weekly reflection, daily reflection default-on, dashboard Docker deployment, cost chart with axes, configurable refresh, AI daily Stage 2.2, Docker entrypoint flock serialization, cwd-unification + cached credential reuse + non-empty completion body (PR #41), scripted offline E2E harness (PR #44)); `v0.9.6` released (local read-only dashboard React SPA + `/api/v1`, PR-based merge flow `target_branch_mode: pr`, router 5→3 intent consolidation + `force_draft` opt-in, DeepSeek-V4 router robustness, weekly trends + richer `/automation_status`, `transcribe-media` skill, `market-briefing` atomic split into 4). See CHANGELOG for per-release detail.
- v0.5 runtime-first complete; v0.6 skill-first + adaptive memory complete (memory replaced by Judge in v0.9.0); v0.7 date-based memory + multi-type runtime + skill evaluation + auth/HITL/operator pass complete.
- Optional LLM router implemented (canonical v2 3-intent contract: `reply` / `artifact` / `repo_update`, organized around "does this modify the source repo?"; legacy 5-intent v1 names normalized for back-compat; `force_draft` opt-in for the artifact draft gate).
- Runtime observability baseline implemented; `WAITING_USER_INPUT` + single-choice `ask_user` HITL implemented; `repair_skill` collapsed into `update_skill` (router decides create-vs-repair via skill registry lookup).
- Image attachment support implemented (Discord download, per-agent handling, temp file lifecycle).
- Codex repo/workspace skill discovery uses official `.agents/skills/`; workspace hint files (`AGENTS.md` / `CLAUDE.md` / `GEMINI.md`) generated from `WORKSPACE_AGENTS.md`.
- Service-layer extraction complete (task, ask, doctor, automation, HITL services).
- Markdown-aware chunker, structured logging, graceful shutdown, error contract, rate-limiting, concurrent isolation tests, log hygiene, restart/recovery tests, upgrade-path validation all implemented.
- First-class `compose.yaml` + Docker scripts (`scripts/docker-*.sh`) + operator guides (EN + CN) + read-only monitoring dashboard (`oma-dashboard`) published.
- Memory system: event-driven Judge model with two-stage dedup, action-based judgment (`add` / `strengthen` / `supersede` / `no_op`), scope-aware bucketed retrieval, daily diary reflection (default-on as of v0.9.5) + weekly reflection (Tuesday 03:00 local default).
- `seattle-metro-housing-watch` and `market-briefing-*` skill contracts iterated; `market-briefing-ai` includes podcast prefetch + AI daily 4-section checkpointing (Stage 2.2) reading paper-digest JSON directly. Stage 3 split the original monolithic `market-briefing` into 4 atomic skills (`-ai` / `-finance` / `-politics` / `-weekly`) sharing the `reports/market-briefing/` output root.
- Automation YAML files carry explicit `skill_name`; per-automation `auto_approve` flag (default off); human-readable risk reasons in DRAFT cards; `/automation_run` manual trigger; persisted automation runtime state survives restart; reply-to-automation-post promotes to follow-up thread.
- Push notifications layer: Bark first; allow-listed event kinds (`mention_owner` / `task_draft` / `task_waiting_merge` / `ask_user` / `automation_complete` / `automation_failed`); off-platform delivery so OS focus/DND can't bury time-critical events.
- Auth: per-provider QR flow (bilibili shipped); cached credential reuse via `get_valid_credential` + auto-injected `--cookies-path` hints (PR #41) — new tasks / chat replies don't trigger redundant QR flows when a valid credential already exists.
- **Test harness (PR #44)**: scripted offline E2E driver under `tests/harness/` with `HarnessChannel(BaseChannel)` + `StubAgent` (cwd-keyed sessions matching real claude `--resume`) + `StubBilibiliAuthProvider` (no network). 3 yaml regression scenarios + smoke-regression guard test. CLI: `python scripts/run_harness.py <yaml>`; pytest: `pytest tests/harness/test_scenarios.py`. Cross-platform-ready (no Discord imports).
...
- [x] **`market-briefing-*`** (original monolithic `market-briefing`, atomized in Stage 3): finance daily expanded to 8 fixed sections (adds China/HK market, US volatility, China property policy); AI daily expanded to 9 sections with Frontier Labs Radar; frontier watchlist (8 labs) with codified rumor discipline; finance/politics boundary rule; `timeout_seconds: 1500`; new `references/finance_watchlist.md` and `references/ai_frontier_watchlist.md`
...
- [ ] **Actually expose the dashboard beyond loopback (pre-1.0)**. PR #48 shipped the in-app token auth (`OMA_DASHBOARD_AUTH_TOKEN` → `Authorization: Bearer ...` + `?token=...`) plus a `monitoring.md` §6.5 walkthrough of three deployment patterns (Tailscale / Cloudflare Tunnel + Access / reverse proxy + Basic Auth + TLS). The actual deployment shape is still loopback-only: `compose.yaml` publishes `127.0.0.1:8080:8080`, `scripts/docker-common.sh` sets `DASHBOARD_BIND_HOST=127.0.0.1`. To use any of the §6.5 patterns the operator has to manually fork those files (or set `OMA_DASHBOARD_BIND_HOST=0.0.0.0`). Pre-1.0 pickup: pick one canonical pattern (likely Tailscale for the single-host single-user defaults; Cloudflare Tunnel + Access as the documented alternative for sharing), ship the corresponding `compose.yaml` overlay or sidecar (`cloudflared` container, Tailscale serve config, etc.), and reduce the operator-side cognitive load from "read §6.5 and pick" to "follow this 3-command recipe". Token auth + docs already cover the security boundary; what's missing is the turnkey deployment.
...
- [ ] Long-output final delivery: once a skill has persisted the full markdown/json to `runtime.reports_dir/...` (e.g. `market-briefing-weekly` cross-domain synthesis writes a 38KB `cross-domain.md`), the agent still re-emits the entire body as an assistant text reply for Discord — re-streaming on-disk content as output tokens, wasting tokens and stuffing minutes of pure output generation into the subprocess wall-clock budget (real case: `bdcf9908d735` weekly on 2026-05-03 — persist succeeded at 18:16, the trailing chat-body generation got killed by the 1500s wall at 18:22). Direction: treat the published path as truth source — agent emits a short summary + path, runtime reads the file and chunks/attaches for delivery. Aligns with the "single published artifact path" semantics in CLAUDE.md. Different axis from the prior markdown-rendering refinement: that one is about *how* to render; this one is about *not* having the agent re-stream content that's already on disk.
...
- [x] `market-briefing-ai` AI people-pool discovery pipeline: detailed discovery rules in SKILL.md, auto-record via `report_store.py persist`
...
- [ ] **Cross-skill output dependency graph** (Stage 4 of `plans/market-briefing-daily-ai-0900-fail-patt-mutable-nest`): cross-skill output references — e.g. `market-briefing-ai`'s `paper_layer` sub-section reading `~/.oh-my-agent/reports/paper-digest/daily/<DATE>.json` after PR #24 — currently rely on implicit cron timing (paper-digest 8:30 → market-briefing-ai 9:00). As more skills compose this way (Stage 2.2 introduced one such link; Stage 3's `market-briefing-*` split expands the surface to 4 sibling skills), an explicit dependency-graph DSL is needed: skill frontmatter `depends_on: [paper-digest]`, scheduler reads the graph and orders cron / holds downstream until upstream completes / propagates failure signals so a downstream consumer reading stale-or-missing JSON gets a clear "upstream failed" signal instead of silently degrading. Today's mitigation is the per-skill `paper_digest_status: missing | stale` field plus `coverage_gaps`, but that's a per-skill convention rather than a system-level guarantee.
- [ ] **Investigate why Claude's `Task` tool isn't firing on aggregation skills** (Stage 4 of the same plan): 3 recent successful deals-scanner runs (`d0c8cfae2b0c`, `07f32bac12e5`, `59860d9c2d34`) had `Task` listed in their JSONL `system / subtype: init` event's `tools` array but **0** subsequent `tool_use name=Task` invocations — the agent did 11–22 sequential `Bash` calls instead. Both `skills/deals-scanner/SKILL.md:149` and `skills/market-briefing-ai/SKILL.md` AI workflow (PR #24) document a "parallel preferred" hint pointing at `Task`, but neither is verifiable in production today. Investigate prompt-shape / model-preference / allowed_tools interaction. Outcome: either the hint becomes effective (and we measure parallel wall-clock wins) or we accept sequential as the actual behavior and remove the unfulfilled hints from SKILL.md to reduce reader confusion.
...
- [ ] **Harness v2 — slash command coverage** (PR #44 follow-up). Today's `HarnessChannel.inject_user_message` only models normal chat. Add `inject_slash_command(name, args)` so scenarios can drive `/task_stop`, `/auth_clear bilibili`, `/automation_run`, etc. Concrete first scenario: owner runs `/task_stop` mid-task → assert task transitions to STOPPED + workspace cleaned up + no further channel writes. Requires modeling Discord slash routing without depending on `discord.app_commands` (slash handlers are wired by GatewayManager via setter hooks; harness can call the same handler stubs).
- [ ] **Harness v2 — real-mode CI integration** (PR #44 follow-up). `--mode real` is wired but raises `NotImplementedError`. Implement: real `ClaudeAgent` / `CodexAgent` + real `BilibiliAuthProvider` constructed when `OMA_HARNESS_ALLOW_REAL=1`. Wire into a nightly CI job that runs `bilibili_cached.yaml` against the real bilibili API + real claude CLI to catch yt-dlp / bilibili API drift (subtitle field renames, etc.). Keep PR-time CI on stub mode for speed + determinism.
- [ ] **Harness v2 — Slack/Feishu HarnessChannel emulation** (PR #44 follow-up; only relevant once those backends start). When Slack adapter work begins, add a `SlackShapedHarnessChannel` mirroring Slack's BlockKit buttons, `ts`-based message ids, `chat.update` edit semantics. The same yaml scenarios (`bilibili_chat_reply_resume.yaml` etc.) should run unmodified — that's the cross-platform contract test paying off.
- [ ] **Boot refactor — extract pure builder phase from `boot.ignite()`** (PR #44 follow-up). Today the harness wires its own component graph following the `runtime_env` fixture pattern (~50 lines duplicated from `boot.py`) instead of reusing `boot.ignite()` because `ignite` owns signal handlers, push-dispatcher daemon, scheduler supervisor, etc. — all noise in tests. Refactor: split into `build_runtime_graph(config) → ComponentGraph` (pure) + `run_forever(graph)` (signal handlers + daemons + shutdown). Harness calls just the builder; production calls both. New components added to either side stay in lockstep.
- [ ] **Cleanup — drop remaining `getattr(channel, ...)` defensive calls** (PR #44 follow-up). GatewayManager + RuntimeService still guard several channel methods with `hasattr` (`set_session_context`, `set_skill_syncer`, `set_runtime_service`, `set_scheduler`, `register_dump_channel`, `ensure_dm_channel`). Once Slack/Feishu lands and these are confirmed to be either always-implemented or have ABC defaults, drop the guards and call directly. Pure cleanup — no behavior change. Sequence with the boot refactor.
- [ ] **macOS Docker bind-mount + SQLite WAL: external readers see stale state while writer is alive** (corrects the prior "post-completion hang" entry merged in PR #28 — that was a misdiagnosis caused by this exact issue). Symptom: querying `~/.oh-my-agent/runtime/runtime.db` from the macOS host (or even via `docker exec` + a fresh `sqlite3.connect()` inside the same container) returns committed-task state that is **minutes-to-hours stale**, missing recent INSERTs / UPDATEs / events that the runtime process has already executed and `await db.commit()`-ed. Confirmed reproducibility on 2026-05-03 with task `4c59b02a6806`: docker logs showed `Runtime task=4c59b02a6806 COMPLETED step=1` at 21:32:19 UTC and Discord successfully received the structured-summary post at the same time, but external SELECTs (host + `docker exec`) showed `status=RUNNING` and zero `task.completed` events for **30+ minutes after the runtime had already moved on** (and was correctly running the next finance-daily task). On graceful shutdown (SIGINT triggers `cooperative shutdown + WAL checkpoint on close` from PR #16) the host immediately saw **all 30+ minutes of accumulated writes appear at once**: `09d739572674` finance task COMPLETED at 21:58:46, plus the missing 4c59b02a6806 status flip and `task.completed` event and `automation_posts` row. The shutdown's `WAL checkpoint complete log_pages=0 checkpointed=0` line shows the WAL was already empty at close, so the writes were not in WAL — they were trapped somewhere between aiosqlite's mmap'd pages and the underlying file inode visible to other readers (Docker Desktop on macOS routes bind-mount IO through VirtioFS / gRPC FUSE, and SQLite's mmap dirty pages aren't msync'd back to the underlying file until the writer process closes). **Operationally this is not a runtime bug** — automations complete correctly, Discord posts land, files persist on disk; it is purely an **observability / diagnostic-tool gotcha** for operators on macOS. Fix surface: (a) document this prominently in the operator guide so we don't waste another half-day diagnosing phantom "hangs" (EN + CN docs/operating-the-bot.md); (b) add a periodic forced `PRAGMA wal_checkpoint(FULL)` to the runtime (every ~30s on a janitor-loop tick) so external readers can see write progress within seconds rather than at writer-close — trades a tiny perf cost for visibility; (c) optionally provide a `make doctor` / `oh-my-agent doctor` host-side helper that does `docker stop && sqlite3 ... && docker start` automatically when an operator wants ground-truth, with the requeue side effect understood; (d) consider disabling SQLite mmap entirely (`PRAGMA mmap_size=0`) on the runtime DB on macOS, accepting some perf loss for cross-process visibility consistency. Leaving (b) as the most likely first fix once we test it doesn't tank perf. **Linux deployments are unaffected** — no FUSE layer, mmap dirty pages are visible to other processes via the page cache.
- [ ] **Auto-rebuild the Docker image when stale vs `Dockerfile` / `docker/entrypoint.sh`** (deferred 2026-05-18). `scripts/docker-common.sh` `oma_docker_ensure_ready` only runs `docker-build.sh` when the image is *missing* (`docker image inspect … || build`). So after a `git pull` that changes `docker/entrypoint.sh` (COPY'd into the image via the Dockerfile) or the `Dockerfile`/deps, plain `scripts/docker-run.sh` keeps using the stale image and the change silently doesn't take effect — the operator must remember a manual `scripts/docker-build.sh` (exactly what #58's entrypoint change required). Pure Python (editable-installed from the bind mount) and frontend-source changes still need no rebuild — only image-baked things do. Sketch: a staleness check in `oma_docker_ensure_ready` (mtime or git-hash of `Dockerfile` + `docker/entrypoint.sh` vs a label baked into the built image) that auto-triggers the rebuild. Surfaced while wrapping up the #56/#57/#58 dashboard work.
