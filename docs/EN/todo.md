...
- [ ] **`_execute_task_loop` post-completion hang on `completion_mode=reply` artifact tasks**: reproducibly observed on `4c59b02a6806` 2026-05-03 (market-briefing-daily-ai-0900 manual `/automation_run`). Both attempts at running this task (original + post-restart requeue) had: (a) claude-cli completed cleanly (`terminal_reason=completed`, exit 0, all 4 sub-section files persisted); (b) DB row stayed `RUNNING` indefinitely (5800s+ for the first, ongoing for the second); (c) **no `task.completed` AND no `task.notification_failed` event** — the COMPLETED branch in `runtime/service.py:3242` enters `await self._notify(...)` but neither returns nor raises (so neither the success path at `service.py:3295` flipping to COMPLETED nor the except path at `service.py:3251` flipping to FAILED ever fires); (d) no `automation_posts` row written; (e) heartbeat `task.agent_progress` events stopped on the 2nd run after the 60s mark (vs ran throughout the 1st run's 18 min) — strongly suggests the entire `_execute_task_loop` asyncio task crashed/cancelled silently around 60s in but the claude subprocess kept running independently and finished on its own. **Repro signature**: requeue-after-hang triggers a 2nd run; the 2nd run is fast (12 turns / 287s / $0.97 thanks to Stage 2.2 checkpoint recovery) but hangs the same way. Other ai-0900 tasks the same day (`4c1fd715086f` 17:15 UTC) and unrelated automations (`bdcf9908d735` weekly's FAILED-via-timeout path, `0186254e737d` youtube weekly) DID complete and post to `automation_posts` normally, so the bug is specific to the COMPLETED-via-reply branch on re-claimed-after-requeue tasks. Across past 30 days of automation tasks, the matching pattern (`agent_log shows terminal_reason=completed + exit 0 BUT db.status != COMPLETED`) was 0 occurrences before today — this is the first repro. Investigation surface: (1) does `requeue_inflight_runtime_tasks` in `memory/store.py:1842` leave any in-memory state in `_running_tasks` / `_task_sources` / `self._workers` inconsistent with the requeued row, such that the post-completion notify path awaits on a future from a dead worker? (2) is the agent's final structured-summary chat reply hitting Discord rate-limit + chunk-message infinite retry? (3) is `_emit_automation_terminal_push` (push notification fan-out) blocking on a never-resolving HTTP future? Need docker stderr / asyncio debug logs from the 21:27-21:35 UTC window to pin down. Workaround until fixed: restart container + manually `update runtime_tasks set status='FAILED', error='hung post-completion' where id='<id>'` to break the requeue loop.
